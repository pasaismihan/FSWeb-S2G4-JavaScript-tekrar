...
function CemberinAlani(cemberYaricap, piParametre) {
  return piParametre * Math.pow(cemberYaricap, 2);
}
...
enbuyuk = sayilar.reduce((toplam, max) => {
  return toplam < max ? max : toplam;
}, 0);
console.log(enbuyuk);

enkucuk = sayilar.reduce((toplam, min) => {
  return toplam > min ? min : toplam;
});
console.log(enkucuk);
...
besyuzdenkucuksayilar = sayilar.filter((sayilar) => {
  return sayilar < 500;
});
...
tekraredensayilar = tekrarArr.map(
  (e) => `${e[0]} sayısı ${e[1]} kere tekrar edilmiştir`
);
